...
#define gObjectSyokudaiTypeSwitchCausesFlameDL ((void*)0x060003A0)
#define gObjectSyokudaiTypeFlameCausesSwitchDL ((void*)0x06000B90)
#define gObjectSyokudaiTypeNoSwitchDL ((void*)0x06000870)
...
void ObjSyokudai_Update(Actor* thisx, PlayState* play2) {
    ...
    // prevent softlock in the goron area with two torch puzzles
    if (play->sceneId == 33 && play->roomCtx.curRoom.num == 7
        && gSaveContext.save.saveInfo.inventory.items[SLOT_ARROW_FIRE] == ITEM_ARROW_FIRE
        && sNumLitTorchesInGroup < 4
    )
        sNumLitTorchesInGroup = 4;
    ...
    if (this->pendingAction != OBJ_SYOKUDAI_PENDING_ACTION_NONE) {
        if (CutsceneManager_GetCurrentCsId() != thisx->csId) {
            ...
            CutsceneManager_Stop(thisx->csId); // XXX hotfix to prevent getting stuck in cutscene
        }
    } else {
