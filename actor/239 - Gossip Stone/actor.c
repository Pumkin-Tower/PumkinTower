...
static InitChainEntry sInitChain[] = {
    ICHAIN_F32(targetArrowOffset, 150, ICHAIN_CONTINUE),
...
void EnGs_Init(Actor* thisx, PlayState* play) {
    ...
    // changes for custom text
    this->unk_195 = this->actor.params;//ENGS_GET_1F(thisx);
    this->switchFlag = 0;//ENGS_GET_SWITCH_FLAG(thisx);
    this->actor.params = ENGS_0;//ENGS_GET_F000(thisx);
...
void EnGs_Draw(Actor* thisx, PlayState* play) {
    ...
    EnGs* this = THIS;
    u32 frames;
    u32 which[] = { 0x060001D0, 0x06003770 };
    ...
    gSPDisplayList(POLY_OPA_DISP++, (void*)which[Player_GetMask(play) == PLAYER_MASK_TRUTH]);
