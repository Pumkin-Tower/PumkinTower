#include "z_en_fz_fire.h"
#include "object_fz.h"
#include "gameplay_keep.h"
#include "0x0084-Burnard.h"
...
// Moving Burnard that can vanish and reappear
...
static InitChainEntry sInitChain[] = {
    //ICHAIN_S8(naviEnemyId, 0x3C, ICHAIN_CONTINUE),
...
void EnFz_ApplyDamage(EnFz* this, PlayState* play) {
    ...
    if (this->isFreezing) {
        if ((this->actor.params < 0) && (this->collider1.base.atFlags & AT_HIT)) {
        ...
        } else if (this->collider1.base.acFlags & AC_HIT && (this->actor.colChkInfo.damageEffect != 0xE)) {
            this->collider1.base.acFlags &= ~AC_HIT;
            ...
            this->collider1.base.acFlags &= ~AC_HIT;
            Vec3f tempPos = this->actor.world.pos;
            tempPos.y = tempPos.y + 30.0f;
            CollisionCheck_SpawnShieldParticlesMetal(play, &tempPos);
        }
    }
}
...
void EnFz_Draw(Actor* thisx, PlayState* play) {
    static Gfx* displayLists[] = {
        (void*)(0x06006088)/* gBurnard_DlOb06000FE0 */,              // Body fully intact           (5 or 6 health)
        (void*)(0x06003DD8)/* gBurnard_DlBurnardBroken1 */, // Top right horn chipped off  (from Freezards perspective)   (3 or 4 health)
        (void*)(0x06004950)/* gBurnard_DlBurnardBroken2 */,         // Entire head chipped off     (1 or 2 health)
    };
    EnFz* this = (EnFz*)thisx;
    s32 pad;
    s32 index;
    ...
    if (this->isActive) {
        ...
        gSPSegment(POLY_XLU_DISP++, 0x08,
                   Gfx_TwoTexScroll(play->state.gfxCtx, G_TX_RENDERTILE, 0, 0, 0x20, 0x40, 1, 0,
                                    (play->state.frames * -10) & 0x7F, 0x20, 0x40));
...
void EnFz_SpawnIceSmokeNoFreeze(EnFz* this, Vec3f* pos, Vec3f* velocity, Vec3f* accel, f32 xyScale) {
    ...
    for (i = 0; i < EN_FZ_EFFECT_COUNT; i++) {
        if (effect->type == 0) {
            ...
            effect->xyScale = xyScale / 20000.0f;
            effect->primAlpha = 0;
            effect->timer = 0;
            break;
        }
...
void EnFz_DrawEffects(EnFz* this, PlayState* play) {
    ...
    for (i = 0; i < EN_FZ_EFFECT_COUNT; i++) {
        if (effect->type > 0) {
            ...
            if (!materialFlag) {
                //gSPDisplayList(POLY_XLU_DISP++, (void*)(gBurnard_MtlBurnardbreath));
                gSPDisplayList(POLY_XLU_DISP++, (void*)(0x060062E8));             
                materialFlag++;
            }

            // improved fading on larger flames
            int opa = effect->primAlpha; // use builtin fade-out
            if (effect->primAlphaState == 0) // fades in on a more aggressive curve
                opa = CLAMP(opa * 2, 0, 200);
            // keep visibility on smaller embers
            if (effect->xyScale < 0.001)
                opa = 255;
            gDPSetPrimColor(POLY_XLU_DISP++, 0, 0, 255, 100, 0, opa);
            ...
            gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(gfxCtx),
                      G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
            //gSPDisplayList(POLY_XLU_DISP++, (void*)(gBurnard_DlBurnardBreath));
            gSPDisplayList(POLY_XLU_DISP++, (void*)(0x060063A0));
            //gDPSetEnvColor(POLY_XLU_DISP++, 255, 50, 0, 200);
        }
