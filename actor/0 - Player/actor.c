...
// pumkin tower version string
#define VERSION "v1.1.0"
#include "commit-hash.h"

// if you need to override that for some reason, just do the following:
//#undef COMMIT
//#define COMMIT 0x9067cea8 // example; it's output from the 'git rev-parse --short HEAD' command
...
//#define WANT_NOCLIP // XXX testing in production, careful not to leave it enabled

// automatic dpad change page
static u8 sGotItem = 0xff;
...
asm("sGameOverTimer = 0x801e1110");
#define GAMEOVER_FULLHEALTH -123
...
#define icon_item_static_yar 0x00a36c10
static u8 dpadPressed[4];
...
static void RefillChateauRomani(void)
{
    // it turns out player doesn't lose chateau romani
    // on game reset, so this isn't needed?
    return;
    
    // if player had chateau romani before, give them it
    if (gSaveContext.save.saveInfo.inventory.items[SLOT_TRADE_COUPLE] != ITEM_NONE)
        gSaveContext.save.saveInfo.inventory.items[SLOT_TRADE_COUPLE] = ITEM_CHATEAU;
}
...
// this complements the new routing system
static void GenericRouteToScene(int scene, int spawn)
{
    // revamped entrance system
    extern SceneEntranceTableEntry sSceneEntranceTable[0x6f];
    asm("sSceneEntranceTable = 0x801C5720;");
    sSceneEntranceTable[0].table[0]->sceneId = scene;
...
// funny enemy descriptions
// it's overengineered, but the goal was to not reuse
// the same strings until each one has been used once
void UseZtargetingTechniques(PlayState* play, Actor* actor)
{
    if (!actor)
        return;
    
    Font* font = &play->msgCtx.font;
    char *start = font->msgBuf.schar + 11;
    char *tmp = start;
    static const char *introBank[] = {
        "Golly gee"
        , "Holy moly"
        , "Oh! MY GOD!" // reference to 'RCP is HUNG UP!! Oh! MY GOD!!'
        , "Holy wow" // obligatory 'wow'
        , "Jinkies"
        , "Well, I'll be"
        , "Hoo boy"
        , "By Jove"
    ...
    static const char *tatlBank[] = {
        ...
        , "God save the Queen?"
        , "It might be a shadow clone?"
        , "No, I can't imagine that"
        , "You don't know about it?"
        , "You might scream in your sleep?"
        , "RCP is HUNG UP?"
    };
    static const char *names[690] = {
        [ACTOR_EN_JSO] = "Garo",
        [ACTOR_EN_TANRON6] = 0,//"Unused invisible enemy. Only draws a circle shadow.",
        [ACTOR_EN_THIEFBIRD] = "Takkuri",
        [ACTOR_EN_RAT] = "Real Bombchu",
        [ACTOR_EN_JSO2] = "Garo Master",
        [ACTOR_EN_RD] = "Hungry Pumkin",//"Redead/Gibdo that cannot talk to the player.",
        [ACTOR_EN_BBFALL] = "Red Bubble",
        [ACTOR_EN_DEKUNUTS] = "Mad Scrub",
        [ACTOR_EN_BIGPO] = "Big Poe",//"Big Poe. Found under Dampe's house and in Beneath the Well",
        [ACTOR_EN_BU] = 0,//"Unused dummied-out enemy",
        [ACTOR_EN_PR2] = "Skullfish",
        [ACTOR_DEMO_SHD] = "",
        [ACTOR_EN_RUPPECROW] = 0,//"Guay Circling Clock Town",
        [ACTOR_EN_DG] = "Dog",
        [ACTOR_BOSS_05] = "Bio Deku Baba",
        [ACTOR_EN_EGOL] = "Eyegore",
        [ACTOR_EN_FIREFLY] = "Keese",//"Keese (Normal, Fire, Ice)",
        [ACTOR_EN_WALLMAS] = "Wallmaster",
        [ACTOR_EN_FIREFLY2] = 0,//"[Empty]",
        ...
        [ACTOR_EN_WIZ_FIRE] = "Wizrobe",//"Wizrobe Fire/Ice Attack",
        [ACTOR_EN_WIZ] = "Wizrobe",
        [ACTOR_OBJ_ROOMTIMER] = 0,
        [ACTOR_EN_FLOORMAS] = "Floormaster",
        [ACTOR_EN_SB] = "Shellblade",
        [ACTOR_EN_ST] = "Skulltula",//"Skulltula (large suspended one)",
        [ACTOR_EN_BAT] = "Bad Bat",
        [ACTOR_EN_KAME] = "Snapper",
        [ACTOR_EN_BUBBLE] = "Shabom",//"Shabom (unused)",
        [ACTOR_EN_SYATEKI_DEKUNUTS] = 0,//"Shooting Gallery Deku Scrub",
        [ACTOR_EN_SYATEKI_WF] = 0,//"Shooting Gallery Wolfos",
        [ACTOR_EN_SLIME] = "Chuchu",
        [ACTOR_EN_RR] = "Like Like",
        [ACTOR_EN_BAGUO] = "Nejiron",
        [ACTOR_EN_KAREBABA] = "Wilted Baba",//"Wilted Deku Baba and Mini Baba",
        [ACTOR_EN_ATTACK_NIW] = "Cucco",//"Attacking Cucco",
        [ACTOR_EN_SKB] = "Stalchild",
        [ACTOR_EN_RAILGIBUD] = "Hungry Pumkin",//"Music Box House - Patrolling Gibdos",
        [ACTOR_EN_CROW] = "Guay",
        [ACTOR_EN_TANRON1] = 0,//"Swarm of Moths in Woodfall Temple",
        [ACTOR_EN_FAMOS] = "Death Armos",//"Death Armos (floating statue enemy in Inverted Stone Tower Temple)",
        [ACTOR_EN_RAIL_SKB] = 0,//"Ikana Graveyard - Circle of Stalchildren",
        [ACTOR_EN_SYATEKI_OKUTA] = 0,//"Shooting Gallery Octorok",
        [ACTOR_EN_PO_SISTERS] = "Poe Sister",//"Poe Sisters (fought in a mini-game in Ikana for a HP)",
        [ACTOR_EN_AM] = "Armos",
        [ACTOR_EN_PR] = "Desbreko",
        [ACTOR_EN_FZ] = "Freezard",
        [ACTOR_EN_PRZ] = "Skullfish",//"Skullfish - Defeated",
        [ACTOR_EN_POH] = "Poe",
        [ACTOR_EN_DINOFOS] = "Dinolfos",
        [ACTOR_EN_TITE] = "Tektite",
        [ACTOR_EN_WDHAND] = "Dexihand",
        [ACTOR_EN_FIRE_ROCK] = 0,//"[Empty]",
        [ACTOR_EN_PP] = "Hiploop",
        [ACTOR_EN_ENCOUNT3] = 0,//"Garo spawner",
        [ACTOR_EN_OKUTA] = "Octorok",
        [ACTOR_EN_DRAGON] = "Deep Python",
        [ACTOR_EN_NEO_REEBA] = "Leever",//"(New) Leevers",
        [ACTOR_EN_SYATEKI_CROW] = "Shooting Gallery Guay",
        [ACTOR_EN_IK] = "Iron Knuckle",
        [ACTOR_EN_TALK_GIBUD] = "Hungry Pumkin",//"Gibdos requesting items Beneath the Well",
        [ACTOR_EN_MKK] = "Boe",//"Black and White Boe (Name origin: Makkurokurosuke)",
    ...
    const char *name = names[actor->id];
    const char *intro = 0;
    const char *tatlBeingTatl = 0;
    static u32 introReset = 0xffffffff;
    static u32 tatlReset = 0xffffffff;
    static u32 introUsed = 0xffffffff;
    static u32 tatlUsed = 0xffffffff;
    int i;
    
    // safety
    if (actor->id >= ARRAY_COUNT(names))
        return;
    
    // reset
    if (introReset == 0xffffffff)
        for (i = introReset = 0; i < ARRAY_COUNT(introBank); ++i)
            introReset |= 1 << i;
    if (tatlReset == 0xffffffff)
        for (i = tatlReset = 0; i < ARRAY_COUNT(tatlBank); ++i)
            tatlReset |= 1 << i;
    if (introUsed == introReset || introUsed == 0xffffffff)
        introUsed = 0;
    if (tatlUsed == tatlReset || tatlUsed == 0xffffffff)
        tatlUsed = 0;
    
    // random
    for (int i = Rand_S16Offset(0, 100); !intro; ++i)
    {
        u8 k = i % ARRAY_COUNT(introBank);
    ...
    for (int i = Rand_S16Offset(0, 100); !tatlBeingTatl; ++i)
    {
        u8 k = i % ARRAY_COUNT(tatlBank);
        
        if (!(tatlUsed & (1 << k)))
        {
            tatlUsed |= 1 << k;
            tatlBeingTatl = tatlBank[k];
        }
    }
    
    // create string
    //sprintf(tmp, "\x05""Wow! Very cool! It's a \x7f""Torch Slug\x05""!\xbf"); // sample
    tmp += sprintf(tmp, "\x05""%s! ", intro);
    if (!name)
        tmp += sprintf(tmp, "\x7fI don't know what that is!\x05\x11""You've never seen one before either?!");
    else
        tmp += sprintf(tmp, "%s's a \x7f""%s\x05""!\x11""What?! %s!", (play->gameplayFrames & 1) ? "That" : "It", name, tatlBeingTatl);
    tmp += sprintf(tmp, "\x11""Stay back & use Z-Targeting techniques!");
    //tmp += sprintf(tmp, "\x11""0x%x/0x%x 0x%x/0x%x", introUsed, introReset, tatlUsed, tatlReset); // debugging
    tmp += sprintf(tmp, "\x7f\xbf");
    
    // character conversion
    for (tmp = start; *tmp != 0xbf; ++tmp)
        if (*tmp == 0x7f)
            *tmp = 0x00;
}
...
static void* Object_GetSegment(ObjectContext* objectCtx, s16 objectId) {
    s32 i;

    for (i = 0; i < objectCtx->numEntries; i++) {
        if (ABS_ALT(objectCtx->slots[i].id) == objectId) {
            return objectCtx->slots[i].segment;
        }
    }
...
// use any item anywhere
static void Interface_LiftSceneRestrictions(PlayState* play) {
    InterfaceContext* interfaceCtx = &play->interfaceCtx;
    //s16 i = 0;
    //u8 currentScene;

    interfaceCtx->restrictions.hGauge = 0;//RESTRICTIONS_GET_HGAUGE(&sRestrictionFlags[i]);
    interfaceCtx->restrictions.bButton = 0;//RESTRICTIONS_GET_B_BUTTON(&sRestrictionFlags[i]);
    interfaceCtx->restrictions.aButton = 0;//RESTRICTIONS_GET_A_BUTTON(&sRestrictionFlags[i]);
    interfaceCtx->restrictions.tradeItems = 0;//RESTRICTIONS_GET_TRADE_ITEMS(&sRestrictionFlags[i]);
    interfaceCtx->restrictions.songOfTime = 0;//RESTRICTIONS_GET_SONG_OF_TIME(&sRestrictionFlags[i]);
    interfaceCtx->restrictions.songOfDoubleTime = 0;//RESTRICTIONS_GET_SONG_OF_DOUBLE_TIME(&sRestrictionFlags[i]);
    interfaceCtx->restrictions.invSongOfTime = 0;//RESTRICTIONS_GET_INV_SONG_OF_TIME(&sRestrictionFlags[i]);
    interfaceCtx->restrictions.songOfSoaring = 0;//RESTRICTIONS_GET_SONG_OF_SOARING(&sRestrictionFlags[i]);
    interfaceCtx->restrictions.songOfStorms = 0;//RESTRICTIONS_GET_SONG_OF_STORMS(&sRestrictionFlags[i]);
    interfaceCtx->restrictions.masks = 0;//RESTRICTIONS_GET_MASKS(&sRestrictionFlags[i]);
    interfaceCtx->restrictions.pictoBox = 0;//RESTRICTIONS_GET_PICTO_BOX(&sRestrictionFlags[i]);
    interfaceCtx->restrictions.all = 0;//RESTRICTIONS_GET_ALL(&sRestrictionFlags[i]);
    
    // don't allow C button items in fishing area
    if (actorAddr(ACTOR_EN_FISHING, 0x809171FC))
        interfaceCtx->restrictions.all = true;
}
...
// (made every treasure chest animation short)
#define GET_ITEM(itemId, objectId, drawId, textId, field, chestAnim) \
    { itemId, field, /* (chestAnim != CHEST_ANIM_SHORT ? 1 : -1) */ -1 * (drawId + 1), textId, objectId }
...
GetItemEntry sGetItemTable[GI_MAX - 1] = {
    ...
    // GI_QUIVER_40
    GET_ITEM(ITEM_QUIVER_40, OBJECT_GI_BOW, GID_BOW, 0x23, GIFIELD(GIFIELD_20 | GIFIELD_NO_COLLECTIBLE, 0),
             CHEST_ANIM_LONG),
    // GI_QUIVER_50
    GET_ITEM(ITEM_QUIVER_50, OBJECT_GI_BOW, GID_BOW, 0x24, GIFIELD(GIFIELD_20 | GIFIELD_NO_COLLECTIBLE, 0),
             CHEST_ANIM_LONG),
    // GI_ARROW_FIRE
    ...
    GET_ITEM(ITEM_RECOVERY_HEART, OBJECT_GI_HEART, GID_RECOVERY_HEART, 0x45, GIFIELD(GIFIELD_NO_COLLECTIBLE, 0),
             CHEST_ANIM_LONG),
    // GI_46
    GET_ITEM(ITEM_MAGIC_JAR_BIG, OBJECT_GI_MAGICPOT, GID_MAGIC_JAR_BIG, 0x55,
             GIFIELD(GIFIELD_20 | GIFIELD_40, ITEM00_MAGIC_JAR_BIG), CHEST_ANIM_SHORT),
    // GI_47
...
void Player_ProcessItemButtons(Player* this, PlayState* play) {
    ...
    // disable B button sword while wearing Aegiker Mask
    if (this->actor.id == ACTOR_PLAYER
        && this->transformation == PLAYER_FORM_HUMAN
        && this->currentMask == PLAYER_MASK_CIRCUS_LEADER
    )
    {
    ...
    if (((this->actor.id == ACTOR_PLAYER) && (this->itemAction > PLAYER_IA_FISHING_ROD)) &&
        ...
        i = ((i >= EQUIP_SLOT_A) && (this->transformation == PLAYER_FORM_FIERCE_DEITY)
             ...
             && this->heldItemAction != PLAYER_IA_BOTTLE_EMPTY
             && this->heldItemAction != PLAYER_IA_FISHING_ROD
             )
                ? EQUIP_SLOT_B
                : i;
        ...
        if (item >= ITEM_FD) {
            ...
            if (this->blastMaskTimer == 0) {
                ...
                // the escape mask kills the player instantly to get them out of a softlock
...
void func_80831F34(PlayState* play, Player* this, PlayerAnimationHeader* anim) {
    ...
    if (this == GET_PLAYER(play)) {
        ...
        // bypass fairy when using game-over mask
        if (this->blastMaskTimer == 0 && Inventory_ConsumeFairy(play)) {
            play->gameOverCtx.state = GAMEOVER_REVIVE_START;
            this->av1.actionVar1 = 1;
        } else {
            play->gameOverCtx.state = GAMEOVER_DEATH_START;
            Audio_StopFanfare(0);
            // this change makes game over respawn faster after player death
            play->gameOverCtx.state = GAMEOVER_DEATH_FADE_OUT; sGameOverTimer = 1;//Audio_PlayFanfare(NA_BGM_GAME_OVER);
...
s32 func_808339D4(PlayState* play, Player* this, s32 damage) {
    ...
    // quarter damage while wearing Giant Mask
    if (this->currentMask == PLAYER_MASK_GIANT) {
        damage >>= 2;
    }

    // double damage if not wearing Fierce Deity Mask
    if (this->transformation != PLAYER_FORM_FIERCE_DEITY)
        damage *= 2;
                ...
                if ((this->itemAction >= PLAYER_IA_MASK_MIN) && (this->itemAction <= PLAYER_IA_MASK_MAX)) {
                    ...
                    gSaveContext.save.equippedMask = this->currentMask;
                           ...
                           ((this->itemAction != this->unk_B2B) &&
                            (false/*(this->itemAction == PLAYER_IA_BOTTLE_BIG_POE)*/ || // allow drinking big poe
            ...
            if ((lockOnActor == NULL) || (lockOnActor == talkActor) || (lockOnActor == var_a1)) {
                if (!(this->stateFlags1 & PLAYER_STATE1_800) ||
                    ...
                    if (((this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) ||
                        ...
                        // custom text
                        if (this->tatlTextId != TATL_HINT_ID_NONE
                            && var_a1
                            && var_a1 == lockOnActor
                        )
                            UseZtargetingTechniques(play, var_a1);
...
// here are placeholders, in case you choose to not use the ported animations
/*
#define gPlayerAnim_link_fishing_throw             gPlayerAnim_link_normal_wait_free
#define gPlayerAnim_link_fishing_wait              gPlayerAnim_link_normal_wait_free
#define gPlayerAnim_link_fishing_reel_left         gPlayerAnim_link_normal_wait_free
#define gPlayerAnim_link_fishing_reel_right        gPlayerAnim_link_normal_wait_free
#define gPlayerAnim_link_fishing_reel_up           gPlayerAnim_link_normal_wait_free
#define gPlayerAnim_link_fishing_reel_down         gPlayerAnim_link_normal_wait_free
#define gPlayerAnim_link_fishing_fish_catch        gPlayerAnim_link_normal_wait_free
#define gPlayerAnim_link_fishing_fish_catch_end    gPlayerAnim_link_normal_wait_free
*/

// the ported animations (see patch/FishingAnims.txt and patch/FishingAnimsKeep.txt)
extern PlayerAnimationHeader gPlayerAnim_link_fishing_fish_catch;
asm("gPlayerAnim_link_fishing_fish_catch = 0x0400cf58;");
extern PlayerAnimationHeader gPlayerAnim_link_fishing_fish_catch_end;
asm("gPlayerAnim_link_fishing_fish_catch_end = 0x0400d050;");
extern PlayerAnimationHeader gPlayerAnim_link_fishing_reel_down;
asm("gPlayerAnim_link_fishing_reel_down = 0x0400d048;");
extern PlayerAnimationHeader gPlayerAnim_link_fishing_reel_left;
asm("gPlayerAnim_link_fishing_reel_left = 0x0400cfd0;");
extern PlayerAnimationHeader gPlayerAnim_link_fishing_reel_right;
asm("gPlayerAnim_link_fishing_reel_right = 0x0400e2f0;");
extern PlayerAnimationHeader gPlayerAnim_link_fishing_reel_up;
asm("gPlayerAnim_link_fishing_reel_up = 0x0400cfd8;");
extern PlayerAnimationHeader gPlayerAnim_link_fishing_throw;
asm("gPlayerAnim_link_fishing_throw = 0x0400d0a0;");
extern PlayerAnimationHeader gPlayerAnim_link_fishing_wait;
asm("gPlayerAnim_link_fishing_wait = 0x0400cfc8;");

#define func_8083A098 func_80836A98
void func_80850E84(Player* this, PlayState* play) {
    if (PlayerAnimation_Update(play, &this->skelAnime) && (this->unk_B28 == 0)) {
...
#define unk_850 av2.actionVar2
#define unk_858 unk_B08
#define unk_85C unk_B0C
#undef unk_858 // TODO look into why these aren't working as expected sometime
#undef unk_85C
#define func_8083721C func_80832F24

    // joystick workaround (copy-pasted from vanilla fishing actor)
    #ifndef unk_858
    static float unk_B08;
    static float unk_B0C;
    Math_SmoothStepToF(&unk_B0C, play->state.input[0].rel.stick_y * 0.02f, 0.3f, 5.0f, 0.0f);
    Math_SmoothStepToF(&unk_B08, play->state.input[0].rel.stick_x * 0.02f, 0.3f, 5.0f, 0.0f);
    ...
    this->unk_B08 = unk_B08;
    this->unk_B0C = unk_B0C;
    #define unk_858 unk_B08
    #define unk_85C unk_B0C
    #endif
    ...
    if ((this->unk_850 != 0) && ((this->unk_858 != 0.0f) || (this->unk_85C != 0.0f))) {
        ...
        PlayerAnimation_BlendToJoint(play, &this->skelAnime, &gPlayerAnim_link_fishing_wait, this->skelAnime.curFrame,
                                   ...
                                   5.0f, fabsf(this->unk_858), this->blendTableBuffer);
        ...
        this->unk_B28 = 2; // unk_860 in oot is unk_B28 in mm
        PlayerAnimation_PlayLoop(play, &this->skelAnime, &gPlayerAnim_link_fishing_wait);
        this->unk_850 = 1;
    }
    ...
    if (this->unk_B28 == 0) {
        ...
        Player_SetAction(play, this, func_80850E84, 0);
        PlayerAnimation_Change(play, &this->skelAnime, &gPlayerAnim_link_fishing_fish_catch, 1.0f, 0.0f, Animation_GetLastFrame(&gPlayerAnim_link_fishing_fish_catch.common), ANIMMODE_ONCE, -6.0f);
    }
#undef unk_850
#undef unk_858
#undef unk_85C
...
s32 func_8083A6C0(PlayState* play, Player* this) {
    if (sPlayerUseHeldItem) {
        ...
        if (this->heldItemAction == PLAYER_IA_FISHING_ROD
            //|| true // XXX no longer required for testing?
        ) {
            ...
            Player_SetAction(play, this, func_80850C68, 0);
            this->unk_B28 = 1;
            Player_StopHorizontalMovement(this);
            PlayerAnimation_PlayOnce(play, &this->skelAnime, &gPlayerAnim_link_fishing_throw);
            return true;
        }
                ...
                if (!(this->stateFlags1 & PLAYER_STATE1_800)) {
                    if (this->getItemId != GI_NONE) {
                        if (CHECK_BTN_ALL(sPlayerControlInput->press.button, BTN_A)) {
                            ...
                            // XXX allow getting items you already have
                            /*
...
void Player_InitCommon(Player* this, PlayState* play, FlexSkeletonHeader* skelHeader) {
    ...
    if (sGameOverTimer == GAMEOVER_FULLHEALTH)
    {
...
void Player_Init(Actor* thisx, PlayState* play) {
    ...
    // clear all music flags to turn off enemy background music
    extern u8 sSeqFlags[128];
    bzero(sSeqFlags, sizeof(sSeqFlags));
    ...
    // NOTE func_18780 / func_8085B384 in mm are func_11D54 / func_80853080 in oot
    ...
    play->unk_1887D = 0;
    play->unk_1887E = 0;
    this->giObjectSegment = ZeldaArena_Malloc(0x3800);
    ...
    // extra items in debug builds
#ifdef WANT_NOCLIP
    gSaveContext.save.saveInfo.inventory.items[SLOT_BOTTLE_1] = ITEM_BUG;
    gSaveContext.save.saveInfo.inventory.items[SLOT_BOTTLE_2] = ITEM_MASK_GIBDO;
#endif
#if DEBUG    
    gSaveContext.save.saveInfo.inventory.items[SLOT_BOTTLE_1] = ITEM_BUG;
    gSaveContext.save.saveInfo.inventory.items[SLOT_BOTTLE_5] = ITEM_BIG_POE;
    gSaveContext.save.saveInfo.inventory.items[SLOT_BOTTLE_6] = ITEM_FISH;
#endif
}
...
void Player_SetDoAction(PlayState* play, Player* this) {
    ...
    if (this->transformation == PLAYER_FORM_GORON) {
        ...
        } else if (this->currentMask == PLAYER_MASK_CIRCUS_LEADER) {
            // Aegiker Crying
            if (CHECK_BTN_ALL(sPlayerControlInput->cur.button, BTN_B)) {
                Audio_PlaySfx(NA_SE_EN_GOLON_KID_CRY - SFX_FLAG);     
            }
...
// adapted from Nax's work, arrow_cycle.c on OoTMM, thanks Nax! <nax.io>
// ported to z64mm by z64me
#if 1
#define GAME_MM

Actor* Actor_Delete(ActorContext* actorCtx, Actor* actor, PlayState* play);

#define UNREACHABLE(X) do{}while(0)

// names
#define gSave gSaveContext.save.saveInfo
#define magicAmount magic
#define itemEquips equips
...
#define PLAYER_IA_ARROW_FIRE   PLAYER_IA_BOW_FIRE
#define PLAYER_IA_ARROW_ICE    PLAYER_IA_BOW_ICE
#define PLAYER_IA_ARROW_LIGHT  PLAYER_IA_BOW_LIGHT
...
static const ArrowInfo kArrowsInfo[] = {
    { ITEM_BOW,         ITS_BOW,         ITEM_BOW,             PLAYER_IA_BOW,          0x2, 0 },
    { ITEM_ARROW_FIRE,  ITS_ARROW_FIRE,  ITEM_BOW_ARROW_FIRE,  PLAYER_IA_ARROW_FIRE,   0x3, 4 },
    { ITEM_ARROW_ICE,   ITS_ARROW_ICE,   ITEM_BOW_ARROW_ICE,   PLAYER_IA_ARROW_ICE,    0x4, 4 },
    { ITEM_ARROW_LIGHT, ITS_ARROW_LIGHT, ITEM_BOW_ARROW_LIGHT, PLAYER_IA_ARROW_LIGHT,  0x5, 8 },
};
...
static int ActorHelper_DoesActorExist(Actor* actor, PlayState* play, int category)
{
    ...
    list = play->actorCtx.actorLists[category].first;
    while (list)
    {
        if (list == actor)
            return 1;
...
void ArrowCycle_Handle(Player* link, PlayState* play)
{
    ...
    if (!arrow)
    {
        ...
        if (gSaveContext.save.playerForm == PLAYER_FORM_HUMAN
            && link->heldItemAction >= PLAYER_IA_BOW
            && link->heldItemAction <= PLAYER_IA_BOW_LIGHT
            && (play->state.input[0].press.button & BTN_R)
            && link->unk_AA5 == PLAYER_UNKAA5_3 // first-person aiming
        )
        {
            for (int i = EQUIP_SLOT_C_LEFT; i <= EQUIP_SLOT_C_RIGHT; ++i)
            {
                ...
                for (itemIndex = 0; itemIndex < 4; ++itemIndex)
                    if (GET_CUR_FORM_BTN_ITEM(i) == kArrowsInfo[itemIndex].icon)
                        break;
...
#endif /* end of arrow_cycle.c */

// fade out the B button when swimming/underwater as Zora Link, but only if you have the fishing rod equipped
static void UpdateFishingRodButtonOpa(Player* this, PlayState* play)
{
    // only if you have the fishing rod equipped
    if (gSaveContext.save.saveInfo.equips.buttonItems[CUR_FORM][EQUIP_SLOT_B] != ITEM_FISHING_ROD)
        return;
    
    if (this->currentBoots == PLAYER_BOOTS_ZORA_UNDERWATER || func_801242B4(this) || (Player_Action_28 == this->actionFunc))
        play->interfaceCtx.bAlpha = 70;
}
...
void Player_UpdateCommon(Player* this, PlayState* play, Input* input) {
    ...
    // title screen loop hack
    if (gSaveContext.gameMode == GAMEMODE_TITLE_SCREEN)
    {
        if (play->csCtx.curFrame == 0x1c0)
        {
    ...
    // fishing rod icon and state persistence across forms
    if (actorAddr(ACTOR_EN_FISHING, 0x809171FC) // simply checks whether fishing actor exists
        && ((EnFishing*)Actor_FindNearby(play, &this->actor, ACTOR_EN_FISHING, ACTORCAT_NPC, 65535))->isFishing
    )
    {
        if (gSaveContext.save.saveInfo.equips.buttonItems[CUR_FORM][EQUIP_SLOT_B] != ITEM_FISHING_ROD)
        {
            gSaveContext.save.saveInfo.equips.buttonItems[CUR_FORM][EQUIP_SLOT_B] = ITEM_FISHING_ROD;
            Interface_LoadItemIconImpl(play, EQUIP_SLOT_B);
            play->interfaceCtx.unk_27E = 1;
        }
    }
    else if (gSaveContext.save.saveInfo.equips.buttonItems[CUR_FORM][EQUIP_SLOT_B] == ITEM_FISHING_ROD)
    {
        int item = GET_CUR_EQUIP_VALUE(EQUIP_TYPE_SWORD);
        
        // XXX for some reason, link gets ice arrows on B button when returning
        //     the fishing rod sometimes; this attempts to mitigate that, but i
        //     don't know how to reproduce the bug, so let's hope it works fine
        ...
        else if (item != ITEM_SWORD_KOKIRI && item != ITEM_SWORD_RAZOR && item != ITEM_SWORD_GILDED)
            item = ITEM_SWORD_KOKIRI;
        
        gSaveContext.save.saveInfo.equips.buttonItems[CUR_FORM][EQUIP_SLOT_B] = item;
        Interface_LoadItemIconImpl(play, EQUIP_SLOT_B);
        Player_UseItem(play, this, ITEM_NONE);
    }
    ...
    if (false)
    {
        s16* D_809171FC = actorAddr(ACTOR_EN_FISHING, 0x809171FC);
        static int x = 0; // hacky delay for testing
        
        if (D_809171FC && !*D_809171FC && ++x == 20)
        {
            play->interfaceCtx.unk_27E = 1;
    ...
    // dpad pressed
    dpadPressed[0] = CHECK_BTN_ANY(input->press.button, BTN_DUP);
    dpadPressed[1] = CHECK_BTN_ANY(input->press.button, BTN_DDOWN);
    dpadPressed[2] = CHECK_BTN_ANY(input->press.button, BTN_DLEFT);
    dpadPressed[3] = CHECK_BTN_ANY(input->press.button, BTN_DRIGHT);
    
    // L button changes dpad ui scale
    if (CHECK_BTN_ANY(input->press.button, BTN_L))
        gSaveContext.unk_3DC2 = !gSaveContext.unk_3DC2;
    
    // TODO delete old code in next commit, turned off for now
    // TODO skip arrow types you don't have
    if (this->heldItemAction >= PLAYER_IA_BOW
        && this->heldItemAction <= PLAYER_IA_BOW_LIGHT
        && CHECK_BTN_ANY(input->press.button, BTN_R)
        && false // turn off this version
    )
    {
         int roll[] = { ITEM_BOW, ITEM_BOW_FIRE, ITEM_BOW_ICE, ITEM_BOW_LIGHT };
         int itemIndex;
         int buttonIndex;
         ...
         for (int i = EQUIP_SLOT_C_LEFT; i <= EQUIP_SLOT_C_RIGHT; ++i)
         {
             ...
             for (itemIndex = 0; itemIndex < 4; ++itemIndex)
                 if (GET_CUR_FORM_BTN_ITEM(i) == roll[itemIndex])
                     break;
         ...
         if (itemIndex < 4)
         {
             int newItem = roll[(itemIndex + 1) % 4];
             
             // set up new arrow type on button
             gSaveContext.save.saveInfo.equips.cButtonSlots[CUR_FORM][buttonIndex] = newItem;
             BUTTON_ITEM_EQUIP(0, buttonIndex) = newItem;
             C_SLOT_EQUIP(0, buttonIndex) = 1;
             Interface_LoadItemIconImpl(play, buttonIndex);
             
             // prevent link from putting bow away
             Player_UseItem(play, this, newItem);
         }
    }
    
    // guarantee environment time-of-day in production builds is the same as debug
    gSaveContext.save.time = CLOCK_TIME(12, 0);
    
    // guarantee player doesn't have dungeon map/compass, to hide minimaps
    gSaveContext.save.saveInfo.inventory.dungeonItems[gSaveContext.mapIndex] = 0;
    
    // fierce deity mask
    //play->sceneId = SCENE_MITURIN_BS; // XXX don't do it this way, it breaks flags on room reset
    // attempted non-working band-aid #rip
    /*
    {
        static u32 bandaid = -1;
        u32 tmp = (GET_CUR_FORM_BTN_ITEM(EQUIP_SLOT_C_LEFT) << 24)
            | (GET_CUR_FORM_BTN_ITEM(EQUIP_SLOT_C_DOWN) << 16)
            | (GET_CUR_FORM_BTN_ITEM(EQUIP_SLOT_C_RIGHT) << 8)
        ;
        
        //if (tmp != bandaid)
        InterfaceContext* interfaceCtx = &play->interfaceCtx;
        {
            u16 *opa = &interfaceCtx->cLeftAlpha;
            bandaid = tmp;
            for (int i = EQUIP_SLOT_C_LEFT; i <= EQUIP_SLOT_C_RIGHT; ++i, ++opa)
            {
    ...
    */
    ...
    if (noclip)
    {
        ...
        if (!CHECK_BTN_ALL(input->cur.button, BTN_L)) {
            ...
            if (CHECK_BTN_ANY(input->cur.button, BTN_DUP | BTN_DLEFT | BTN_DDOWN | BTN_DRIGHT)) {
                s16 angle;
                s16 temp;
                ...
                if (CHECK_BTN_ALL(input->cur.button, BTN_DDOWN)) {
                    angle = temp + 0x8000;
                } else if (CHECK_BTN_ALL(input->cur.button, BTN_DLEFT)) {
                    angle = temp + 0x4000;
    ...
    if ((CHECK_BTN_ALL(play->state.input[0].press.button, BTN_Z)
        ...
        || gSaveContext.gameMode == GAMEMODE_TITLE_SCREEN // automatic jump to map select
    )
    {
        // default selection
        if (gSaveContext.gameMode == GAMEMODE_TITLE_SCREEN)
        {
            ...
            /*
            const int count = 143;
            dREG(80) = (91 - 1); // lottery shop
            dREG(80) = (26 - 1); // goron graveyard (fishing)
            //dREG(80) = (110 - 1); // ice floor
            dREG(80) = (91 - 1) - 8; // mayor's residence (intermission)
            //dREG(80) = (101 - 1); // goron floor
            dREG(81) = (dREG(80) + count) % count;
            */
            
            // new map select
            // sceneId * -1 to signal initialization to the specified scene
            dREG(80) = -1 * 78; // fishing
            dREG(80) = -1 * 9; // flame circle test room
        }
    ...
    // testing big poe drinking
    //static int x = 0; if (++x < 10) gSaveContext.save.saveInfo.inventory.items[SLOT_BOTTLE_1] = ITEM_BIG_POE;
    
    // L to Levitate (Moon Jump / Moonjump)
    if (CHECK_BTN_ALL(input->cur.button, BTN_L))
    	this->actor.velocity.y = 6.34375;
    
    // press Dpad-Right to cancel cutscenes
    if (CHECK_BTN_ALL(play->state.input[0].press.button, BTN_DRIGHT))
        play->csCtx.state = CS_STATE_STOP;
    #else
    // jump to custom title screen and gameplay
    if (gSaveContext.gameMode == GAMEMODE_FILE_SELECT)
    {
        static u8 isOwlSave[] __attribute__ ((aligned (8))) = { 1, 1, 1, 1 };
        //static u8 saveBuf[SAVE_BUFFER_SIZE];
        //static SramContext sramCtx = { .saveBuf = saveBuf };
        char *name = &((u8*)play->sramCtx.saveBuf)[0x2C];
        int scene = 111; // royal garden
        int spawn = 0;
        
        // load a save, if applicable
        gSaveContext.fileNum = 0;
        
        // read from flash
        bzero(play->sramCtx.saveBuf, SAVE_BUFFER_SIZE);
        int sp64 = (gSaveContext.fileNum + 2) * 2;
        SysFlashrom_ReadData(play->sramCtx.saveBuf, gFlashSaveStartPages[sp64], gFlashSaveNumPages[sp64]);
        ...
        )
        {
            ...
            // defaults
            name = gSaveContext.save.saveInfo.playerData.playerName;
            gSaveContext.save.playerForm = PLAYER_FORM_HUMAN;
            gSaveContext.save.saveInfo.playerData.health = 0x70;
            gSaveContext.save.saveInfo.playerData.healthCapacity = 0x70;
            gSaveContext.save.saveInfo.playerData.isMagicAcquired = true;
            gSaveContext.save.hasTatl = true; // need Tatl for hints to work
            gSaveContext.options.zTargetSetting = 1; // Z-Targeting 'Hold' option
            Lib_MemCpy(name, "ptower\0""P", 8);
            RefillChateauRomani();
        }
        else
        {
            Sram_OpenSave((void*)(((uintptr_t)isOwlSave) - offsetof(FileSelectState, isOwlSave)), &play->sramCtx);
            //extern u32 gFlashSaveSizes[8];
            //Lib_MemCpy(&gSaveContext, play->sramCtx.saveBuf, sizeof(Save));
            //func_801457CC(&play->state, &play->sramCtx);
            
            // hack for persistence across reboots
            gSaveContext.options.zTargetSetting = gSaveContext.save.saveInfo.unk_F40;
            gSaveContext.save.saveInfo.unk_F40 = 0;
            
            // spawn at appropriate feather statue
            if (!gSaveContext.save.owlSaveLocation) // backwards compatibility for play-testers
                gSaveContext.save.owlSaveLocation = 0x1205;
            scene = gSaveContext.save.owlSaveLocation >> 8;
            spawn = gSaveContext.save.owlSaveLocation  & 0xff;
            
            // turn off owl spawn text
            gSaveContext.save.isOwlSave = false;
            
            // prevent player from spawning wearing transformation masks
            switch (gSaveContext.save.equippedMask)
            {
        ...
        GenericRouteToScene(scene, spawn);
        gSaveContext.nextCutsceneIndex = gSaveContext.save.cutsceneIndex = 0;
        gSaveContext.sceneLayer = 0;
        gSaveContext.save.time = CLOCK_TIME(8, 0);
        ...
        STOP_GAMESTATE(&play->state);
        SET_NEXT_GAMESTATE(&play->state, DayTelop_Init, sizeof(DayTelopState));//x
    }
    #endif
...
void Player_Update(Actor* thisx, PlayState* play) {
    ...
    // use any item anywhere
    Interface_LiftSceneRestrictions(play);
    
    // R button changes arrow type while holding bow
...
void Player_DrawGameplay(PlayState* play, Player* this, s32 lod, Gfx* cullDList,
    ...
    // Pumpkin Tunic
    u8 *object;
    if ((object = Object_GetSegment(&play->objectCtx, OBJECT_LINK_CHILD)))
    {
        static u16 off[] = {
            0xb39c, 0xb8c4, 0xbdcc, 0xbfa4, 0xc064, 0xc66c,
            0xcae4, 0xcd1c, 0xcea4, 0xd1ec, 0xd374
        };
        static u32 colors[] = {
            0x620163ff, // Hungry Pumkin Purple instead
            0xffbb00ff, // orange tunic
            0x1e691bff, // green tunic
            0x202020ff, // black Owl Mask tunic
            0xffffffff, // white Bunny Hood tunic
        };
        int mask = Player_GetMask(play);
        int which =
            mask == PLAYER_MASK_GIBDO ? 1
            : mask == PLAYER_MASK_CIRCUS_LEADER ? 2
            : mask == PLAYER_MASK_TRUTH ? 3
            : mask == PLAYER_MASK_BUNNY ? 4
            : 0
        ;
        u32 color = colors[which];
        
        for (int i = 0; i < ARRAY_COUNT(off); ++i)
        {
            u32 *tmp = (void*)(object + off[i]);
    ...
    if (gSaveContext.gameMode == GAMEMODE_TITLE_SCREEN)
    {
        DrawDebugText(play, 44, 16, -1, "%s %s %08x", __DATE__, __TIME__, COMMIT);
        DrawDebugText(play, 136, 214, -1, VERSION);
        DrawDebugText(play, (320 - (12 *  8)) / 2, 226, -1, "www.z64.chat");
    }
#endif
    ...
    if (play->msgCtx.currentTextId)
        DrawDebugText(play, 128, 224, -1, "MESSAGE 0x%04X\n", play->msgCtx.currentTextId);
    // distance to actor test
    else if (false)
    {
        Actor* inst = 0;
        
        // using distance to spawn point here, but adapt it as needed
        // (home = spawn point, world = actual position)
        if ((inst = Actor_FindNearby(play, &this->actor, 31, ACTORCAT_ENEMY, 65535)))
            DrawDebugText(play, 128, 224, -1, "distance: %f\n", Math3D_Vec3f_DistXYZ(&inst->home.pos, &this->actor.world.pos));
    }
    // actor rotation test
    else if (false)
    {
        ...
        if ((inst = Actor_FindNearby(play, &this->actor, ACTOR_OBJ_SWITCH, ACTORCAT_SWITCH, 65535)))
            DrawDebugText(play, 128, 224, 0xff, "HOME.ROT 0x%04X\n", inst->home.rot.x);
    }
    
    // testing debug text
    //DebugCamera_ScreenText(1, 1, 0, "hello");
    //DrawDebugText(play, 128, 128, -1, "hello %d\n", 123);
#endif
...
/* d-pad ui courtesy of z64me <z64.me> */
static void z64dpadMajora(PlayState* play, Player *this)
{
	static const unsigned char dpad_ci4[] __attribute__ ((aligned (8))) =
	{
		...
		
	};
	InterfaceContext *p = &play->interfaceCtx;
	Gfx *buf = play->state.gfxCtx->overlayBuffer;// &ZQDL(gl, overlay);
	
	// turn this off while no-clipping
#ifdef WANT_NOCLIP
	if (noclip)
		return;
#endif
	...
	#define MM_U_1_0 1
	#if OOT_DEBUG
		gSPDisplayList(OVERLAY_DISP++, 0x801269D0);
	#elif	OOT_U_1_0
		gSPDisplayList(OVERLAY_DISP++, 0x800F84A0);
	#elif MM_U_1_0
		gSPDisplayList(OVERLAY_DISP++, 0x801C1640);
	#endif
	gDPSetCombineLERP(
		OVERLAY_DISP++
		, PRIMITIVE
	...
	gDPSetPrimColor(OVERLAY_DISP++, 0, 0, -1, -1, -1, p->aAlpha);
	gDPSetEnvColor(OVERLAY_DISP++, 0, 0, 0, 255);
	gDPPipeSync(OVERLAY_DISP++);
	static u32 dekuMask[32 * 32] __attribute__ ((aligned (8)));
	static u32 goronMask[32 * 32] __attribute__ ((aligned (8)));
	static u32 zoraMask[32 * 32] __attribute__ ((aligned (8)));
	static u32 deityMask[32 * 32] __attribute__ ((aligned (8)));
	static u32 aegikerMask[32 * 32] __attribute__ ((aligned (8)));
	static u32 lensOfTruth[32 * 32] __attribute__ ((aligned (8)));
	static u32 owlMask[32 * 32] __attribute__ ((aligned (8)));
	static u32 pumpkinMask[32 * 32] __attribute__ ((aligned (8)));
	static bool hasLoadedIcons = false;
	if (!hasLoadedIcons)
	{
		hasLoadedIcons = true;

		CmpDma_LoadFile(icon_item_static_yar, 50, dekuMask, 0x1000);
		CmpDma_LoadFile(icon_item_static_yar, 51, goronMask, 0x1000);
		CmpDma_LoadFile(icon_item_static_yar, 52, zoraMask, 0x1000);
		CmpDma_LoadFile(icon_item_static_yar, 53, deityMask, 0x1000);

		CmpDma_LoadFile(icon_item_static_yar, 61, aegikerMask, 0x1000);
		CmpDma_LoadFile(icon_item_static_yar, 14, lensOfTruth, 0x1000);
		CmpDma_LoadFile(icon_item_static_yar, 54, owlMask, 0x1000);
		CmpDma_LoadFile(icon_item_static_yar, 65, pumpkinMask, 0x1000);
	}
	
	/* for each icon */
	const float scale = 1.0f;//gSaveContext.unk_3DC2 ? 1.0f : 0.75f;
	const int diffSquare = 40 * scale;
	const int diffSquareHalf = 20 * scale;
	struct {
		u32 *icon;
		int check;
		int itemId;
		...
		{ dekuMask, ITEM_MASK_DEKU, PLAYER_MASK_DEKU, 0, -2, diffSquare }
		, { deityMask, ITEM_MASK_FIERCE_DEITY, PLAYER_MASK_FIERCE_DEITY, 1, -diffSquare / 2, -diffSquare / 2 }
		, { goronMask, ITEM_MASK_GORON, PLAYER_MASK_GORON, 2, diffSquare, 0 }
		, { zoraMask, ITEM_MASK_ZORA, PLAYER_MASK_ZORA, 3, 0, 0 }
		
		, { aegikerMask, ITEM_MASK_CIRCUS_LEADER, PLAYER_MASK_CIRCUS_LEADER, 0, -2, diffSquare }
		, { lensOfTruth, ITEM_LENS_OF_TRUTH, PLAYER_IA_LENS_OF_TRUTH, 1, -diffSquare / 2, -diffSquare / 2 }
		, { owlMask, ITEM_MASK_TRUTH, PLAYER_MASK_TRUTH, 2, diffSquare, 0 }
		, { pumpkinMask, ITEM_MASK_GIBDO, PLAYER_MASK_GIBDO, 3, 0, 0 }
	};
	
	// got a new item, change dpad to other variation accordingly, if applicable
	if (sGotItem != 0xff)
	{
		for (int i = 0; i < ARRAY_COUNT(advance); ++i)
		{
			if (sGotItem == advance[i].check)
			{
				gSaveContext.unk_3DC2 = i / 4;
				break;
			}
		}
		
		// hotfix nut/stick upgrade text
		if (sGotItem == ITEM_DEKU_NUT_UPGRADE_40
		    || sGotItem == ITEM_DEKU_STICK_UPGRADE_30
		)
		{
			const char *hack = "\xbf";
			...
			switch (sGotItem)
			{
				case ITEM_DEKU_NUT_UPGRADE_40:
					hack = "You upgraded your \x01""Deku Nuts\x00""!\x11""Now you can carry 40!\xbf";
					break;
				
				case ITEM_DEKU_STICK_UPGRADE_30:
					hack = "You upgraded your \x01""Deku Sticks\x00""!\x11""Now you can carry 30!\xbf";
					break;
			}
			
			TextOverride(play, hack);
		}
		sGotItem = 0xff;
	}
	
	// set dpad page = 1 if you don't have any items from page 0
	// (this doesn't solve any bugs, just prevents players from hiding dpad)
	if (gSaveContext.unk_3DC2 == 0
	    && INV_CONTENT(advance[0].check) != advance[0].check
	    && INV_CONTENT(advance[1].check) != advance[1].check
	    && INV_CONTENT(advance[2].check) != advance[2].check
	    && INV_CONTENT(advance[3].check) != advance[3].check
	)
	    gSaveContext.unk_3DC2 = 1;
	
	int advanceStart = gSaveContext.unk_3DC2 ? 4 : 0;
	int advanceEnd = advanceStart + 4;
	int dpadWithL = 0;
	
	// show L button if you have two dpads
	// (this makes assumptions)
	for (int i = 0; i < ARRAY_COUNT(advance); ++i)
	{
		if (INV_CONTENT(advance[i].check) == advance[i].check)
		{
			if (i < 4)
				dpadWithL = (32 * 32) / 2;
		}
	...
	float screenX = 32;
	float screenY = 240 - 55;//78;
	const float iconSquare = 16 * scale;
	
	// hide d-pad if player is missing all masks
	screenX = 1000;
	for (int i = advanceStart; i < advanceEnd; ++i)
		if (INV_CONTENT(advance[i].check) == advance[i].check)
			screenX = 320 - 40;
	...
	gDPPipeSync(OVERLAY_DISP++);
  	gSPSetOtherMode(OVERLAY_DISP++, G_SETOTHERMODE_H, G_MDSFT_TEXTFILT, 2, 0x00000000);
  	gDPSetTextureImage(OVERLAY_DISP++, G_IM_FMT_RGBA, G_IM_SIZ_16b, 1, dpad_ci4);
  	gDPTileSync(OVERLAY_DISP++);
  	gDPSetTile(OVERLAY_DISP++, G_IM_FMT_RGBA, 0, 0, 0x100, G_TX_LOADTILE, 0, 0, 0, 0, 0, 0, 0);
  	gDPLoadSync(OVERLAY_DISP++);
  	gDPLoadTLUTCmd(OVERLAY_DISP++, G_TX_LOADTILE, 255);
  	gSPSetOtherMode(OVERLAY_DISP++, G_SETOTHERMODE_H, G_MDSFT_TEXTLUT, 2, 0x00008000);
		gDPLoadTextureBlock_4b(
			OVERLAY_DISP++
			, (dpad_ci4 + 0x20 + dpadWithL), G_IM_FMT_CI
			, 32, 32
			, 0
		...
		gSPTextureRectangle(
			...
			, qs102((screenX - 3)+ 20.0f * scale) & ~3
			, qs102((screenY - 2)+ diffSquareHalf + 20.0f * scale) & ~3
			, G_TX_RENDERTILE
			, qu105(0), qu105(0)
			, qu510(32.0f/*imgW*/ / (20.0f * scale))
			, qu510(32.0f/*imgH*/ / (20.0f * scale))

		);
	/* disable palette now */
	//gDPFullSync(OVERLAY_DISP++); // not necessary?
	gSPSetOtherMode(OVERLAY_DISP++, G_SETOTHERMODE_H, G_MDSFT_TEXTLUT, 2, G_TT_NONE);

	for (int i = advanceStart; i < advanceEnd; ++i)
	{
		// player has this mask
		if (INV_CONTENT(advance[i].check) == advance[i].check)
		{
			gDPLoadTextureBlock(
				OVERLAY_DISP++
				, advance[i].icon, G_IM_FMT_RGBA, G_IM_SIZ_32b
				, 32, 32
				, 0
				, G_TX_CLAMP, G_TX_CLAMP
				, G_TX_NOMASK, G_TX_NOMASK
			...
			if (dpadPressed[advance[i].button])
			{
				// use the items
				int itemId = Player_MaskIdToItemId(advance[i].itemId - 1);
				
				// special case
				if (advance[i].check == ITEM_LENS_OF_TRUTH)
					itemId = advance[i].check;
				
				Player_UseItem(play, this, itemId);
				
				// test button press logic
				//static int pixel = 0;
				//advance[i].icon[pixel++] = -1;
			}
		}
...
void Player_Draw(Actor* thisx, PlayState* play) {
    ...
    z64dpadMajora(play, this);
    UpdateFishingRodButtonOpa(this, play);
...
void Player_Destroy(Actor* thisx, PlayState* play) {
    ...
    // player died
    if (play->gameOverCtx.state == GAMEOVER_DEATH_FADE_OUT + 1)
        sGameOverTimer = GAMEOVER_FULLHEALTH;
}
...
s32 func_808482E0(PlayState* play, Player* this) {
    ...
    if (this->av1.actionVar1 == 0) {
        GetItemEntry* giEntry = &sGetItemTable[this->getItemId - 1];
        u8 prevForm = gSaveContext.save.playerForm;
        sGotItem = giEntry->itemId;
        
        // it's possible to get chateau romani but not actually get it
        // because all your bottles are already full, so here's a hotfix
        if (sGotItem == ITEM_CHATEAU)
        {
            int i = SLOT_TRADE_COUPLE;
            
            /*
            // decided not to do it this way because it's too finicky
            for (i = SLOT_BOTTLE_1; i <= SLOT_BOTTLE_6; ++i)
                if (gSaveContext.save.saveInfo.inventory.items[i] == ITEM_BOTTLE
                    || gSaveContext.save.saveInfo.inventory.items[i] == ITEM_NONE
                )
                    break;
            
            // chateau romani won't fit, so put it here instead
            if (i > SLOT_BOTTLE_6)
            {
                i = SLOT_TRADE_COUPLE;
                gSaveContext.save.saveInfo.inventory.items[i] = sGotItem;
            }
            */
        ...
        // bugfix for getting gilded sword as link transformations
        if (sGotItem == ITEM_SWORD_GILDED && prevForm != PLAYER_FORM_HUMAN)
            gSaveContext.save.playerForm = PLAYER_FORM_HUMAN;

        this->av1.actionVar1 = 1;
        Message_StartTextbox(play, giEntry->textId, &this->actor);
        
        // account for custom chateau romani slot
        if (sGotItem != ITEM_CHATEAU)
            Item_Give(play, giEntry->itemId);
        
        // restore it, just in case a hotfix changed it somewhere
        gSaveContext.save.playerForm = prevForm;
        
        // double magic upgrade: now the game's twice as good!
        if (this->getItemId == GI_46)
        {
...
void Player_Action_13(Player* this, PlayState* play) {
    ...
    if (this->currentMask == PLAYER_MASK_CIRCUS_LEADER
...
void Player_Action_67(Player* this, PlayState* play) {
    ...
    if (PlayerAnimation_Update(play, &this->skelAnime)) {
        if (this->av2.actionVar2 == 0) {
            if (this->itemAction == PLAYER_IA_BOTTLE_BIG_POE) {
                // jinx link if he drinks big poe
                gSaveContext.jinxTimer = 1200;
            }
            else if (this->itemAction == PLAYER_IA_BOTTLE_POE) {
        ...
        if (!CHECK_BTN_ALL(sPlayerControlInput->cur.button, BTN_A)) {
            if (func_80856000(play, this)) {
                ...
                if (this->av2.actionVar2 >= 10) {
                    this->unk_B48 = 2700.0f; // gold deku flower
                } else {
                    this->unk_B48 = 1450.0f; // deku flower
                }
                
                // highly customizable flower launch distance
                Actor* wow = Actor_FindNearby(play, &this->actor, ACTOR_OBJ_ETCETERA, ACTORCAT_BG, 32);
                if (wow)
                {
                    u8 dist = wow->params & 0x7f;
                    
                    // custom distance
                    if (dist != 0 && dist != 0x7f)
                        this->unk_B48 = dist * 100.0f;
                }
...
u32 PlayerFunctions[] = {
    0x506c6179, 0x65724675, 0x6e637469, 0x6f6e733a // "PlayerFunctions:"
    , (u32)Player_Init, (u32)Player_Destroy, (u32)Player_Update, (u32)Player_Draw
};
