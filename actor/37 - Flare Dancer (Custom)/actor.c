...
#define FLAGS (ACTOR_FLAG_TARGETABLE | ACTOR_FLAG_2 | 0/*ACTOR_FLAG_400*/ | ACTOR_FLAG_200)
...
static CollisionCheckInfoInit/*CollisionCheckInfoInit2*/ sColChkInit = { 24, 2, 25, /*25,*/ MASS_IMMOVABLE };
...
static AnimationInfo sAnimationInfo[] = {
    { (void*)(0x060010B4), 1.0f, 0.0f, -1.0f, ANIMMODE_ONCE_INTERP, 0.0f },
    { (void*)(0x06005C64), 1.0f, 0.0f, -1.0f, ANIMMODE_ONCE_INTERP, -10.0f },
    { (void*)(0x06006044), 0.0f, 0.0f, -1.0f, ANIMMODE_ONCE_INTERP, -10.0f },
    { (void*)(0x06006A18), 1.0f, 0.0f, -1.0f, ANIMMODE_LOOP_INTERP, -10.0f },
    { (void*)(0x06006b64), 0.0f, 0.0f, -1.0f, ANIMMODE_ONCE_INTERP, -10.0f },
};

/**
 * Finds the first actor instance of a specified ID and category if there is one.
 */
static Actor* Actor_Find(ActorContext* actorCtx, s32 actorId, s32 actorCategory, const int which) {
...
static void func_8002DE04(PlayState* play, Actor* actorA, Actor* actorB) {
    ArmsHook* hookshot = (ArmsHook*)Actor_Find(&play->actorCtx, ACTOR_ARMS_HOOK, ACTORCAT_ITEMACTION, 0);

    hookshot->grabbed = actorB;
    hookshot->unk1FC.x = 0.0f;
    hookshot->unk1FC.y = 0.0f;
    hookshot->unk1FC.z = 0.0f;
    actorB->flags |= ACTOR_FLAG_2000;
    actorA->flags &= ~ACTOR_FLAG_2000;
}
...
s32 EnFd_SpawnCore(EnFd* this, PlayState* play) {
    ...
    if (Actor_SpawnAsChild(&play->actorCtx, &this->actor, play, 44, this->corePos.x, this->corePos.y,
...
void EnFd_SpawnChildFire(EnFd* this, PlayState* play, s16 fireCnt, s16 color) {
    ...
    for (i = 0; i < fireCnt; i++) {
        ...
        Actor_SpawnAsChild(&play->actorCtx, &this->actor, play, 41, this->actor.world.pos.x,
...
s32 EnFd_CheckHammer(EnFd* this, PlayState* play) {
    if (this->actionFunc == EnFd_Reappear || this->actionFunc == EnFd_SpinAndGrow ||
        ...
        return false;
    } else if (play->actorCtx.unk2 != 0 && this->actor.xzDistToPlayer < 300.0f && this->actor.playerHeightRel < 60.0f) {
        return true;
    } else {
        return false;
    }
...
s32 EnFd_ColliderCheck(EnFd* this, PlayState* play) {
    ...
    // hotfix: react to nearby explosions
    // TODO: maybe do this correctly sometime
    for (int i = 0; i < 100 && this->coreActive == false; ++i)
    {
        EnBom* bomb = (EnBom*)Actor_Find(&play->actorCtx, ACTOR_EN_BOM, ACTORCAT_EXPLOSIVES, i);
        
        if (!bomb || bomb->timer)
            break;
        
        Vec3f a = bomb->actor.world.pos;
        ...
        if ((x * x + z * z) < (dist * dist) && (y * y) < (distY * distY))
        {
            EnFd_SpawnCore(this, play);
    ...
    if (this->collider.base.acFlags & AC_HIT || EnFd_CheckHammer(this, play)) {
        ...
        func_800B8D98(play, &this->actor, this->actor.speed + 2.0f, this->actor.yawTowardsPlayer, 6.0f);
    }
    return false;
}
...
void EnFd_Init(Actor* thisx, PlayState* play) {
    ...
    SkelAnime_InitFlex(play, &this->skelAnime, (void*)(0x06005810), NULL, this->jointTable, this->morphTable, 27);
...
void EnFd_Reappear(EnFd* this, PlayState* play) {
    ...
    Actor_ChangeAnimationByInfo(&this->skelAnime, sAnimationInfo, ENFD_ANIM_0);
...
void EnFd_SpinAndGrow(EnFd* this, PlayState* play) {
    if (Animation_OnFrame(&this->skelAnime, this->skelAnime.endFrame)) {
        ...
        Actor_ChangeAnimationByInfo(&this->skelAnime, sAnimationInfo, ENFD_ANIM_1);
...
void EnFd_JumpToGround(EnFd* this, PlayState* play) {
    if ((this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) && !(this->actor.velocity.y > 0.0f)) {
        ...
        this->actor.speed = 0.0f;
        this->actor.world.rot.y = this->actor.shape.rot.y;
        Actor_ChangeAnimationByInfo(&this->skelAnime, sAnimationInfo, ENFD_ANIM_2);
...
void EnFd_Land(EnFd* this, PlayState* play) {
    ...
    if (Animation_OnFrame(&this->skelAnime, this->skelAnime.endFrame)) {
        ...
        Actor_ChangeAnimationByInfo(&this->skelAnime, sAnimationInfo, ENFD_ANIM_4);
...
void EnFd_SpinAndSpawnFire(EnFd* this, PlayState* play) {
    ...
    if ((this->spinTimer < 31) && (this->invincibilityTimer == 0)) {
        Actor_PlaySfx_Flagged(&this->actor, NA_SE_EN_NPC_APPEAR - SFX_FLAG);
    } else {
        Actor_PlaySfx_Flagged(&this->actor, NA_SE_EN_LAST3_KOMA_OLD - SFX_FLAG);
    }
    ...
    if (DECR(this->spinTimer) != 0) {
        ...
        if (this->actor.shape.rot.y == this->actor.world.rot.y) {
            ...
            Actor_ChangeAnimationByInfo(&this->skelAnime, sAnimationInfo, ENFD_ANIM_3);
...
void EnFd_Update(Actor* thisx, PlayState* play) {
    ...
    // delayed to fix mini-boss music misbehaving on void-out
/*     if (this->firstUpdateFlag != 20) {
        this->firstUpdateFlag += 1;
        if (this->firstUpdateFlag == 20)
...
void EnFd_Draw(Actor* thisx, PlayState* play) {
    ...
    if (this->actionFunc != EnFd_Reappear && !(this->fadeAlpha < 0.9f)) {
        ...
        gSPSegment(POLY_XLU_DISP++, 0x9, D_801AEF88);

        POLY_XLU_DISP =
            SkelAnime_DrawFlex(play, this->skelAnime.skeleton, this->skelAnime.jointTable, this->skelAnime.dListCount,
                               (OverrideLimbDraw)EnFd_OverrideLimbDraw, (PostLimbDraw)EnFd_PostLimbDraw, (void*)this, POLY_XLU_DISP);
    }
    ...
    for (i = 0; i < EN_FD_EFFECT_COUNT; i++, eff++) {
        if (eff->type == FD_EFFECT_FLAME) {
            if (!materialFlag) {
                ...
                gSPDisplayList(POLY_XLU_DISP++, (void*)(0x06007928));
            ...
            gSPSegment(POLY_XLU_DISP++, 0x8, SEGMENTED_TO_K0(dustTextures[idx]));
            gSPDisplayList(POLY_XLU_DISP++, (void*)(0x06007938));
        }
    }
    CLOSE_DISPS(play->state.gfxCtx);
}
...
void EnFd_DrawEffectsDots(EnFd* this, PlayState* play) {
    ...
    for (i = 0; i < EN_FD_EFFECT_COUNT; i++, eff++) {
        if (eff->type == FD_EFFECT_DOT) {
            if (!materialFlag) {
                Gfx_SetupDL25_Xlu(play->state.gfxCtx);
                gSPDisplayList(POLY_XLU_DISP++, (void*)(0x060079F8));
                materialFlag = true;
            }
            ...
            gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(play->state.gfxCtx),
                      G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
            gSPDisplayList(POLY_XLU_DISP++, (void*)(0x06007A78));
        }
    }
