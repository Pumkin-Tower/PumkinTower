...
// custom LUT
static u16 sTreasureLUT[0x7f] = {
    [0x71] = GI_MASK_GIBDO
    , [0x72] = GI_MASK_CIRCUS_LEADER
    , [0x73] = GI_MASK_BLAST
    , [0x40] = GI_MASK_TRUTH
};
...
void EnBox_Init(Actor* thisx, PlayState* play) {
    ...
    // override certain items with certain other items
    if (sTreasureLUT[this->getItemId])
        this->getItemId = sTreasureLUT[this->getItemId];
...
void EnBox_AppearSwitchFlag(EnBox* this, PlayState* play) {
    ...
    if (Flags_GetSwitch(play, this->switchFlag)) {
        if (CutsceneManager_IsNext(this->csId1)) {
          //CutsceneManager_StartWithPlayerCs(this->csId1, &this->dyna.actor); // bandaid to keep cutscenes from happening
