...
#define FLAGS (ACTOR_FLAG_TARGETABLE | ACTOR_FLAG_2 | ACTOR_FLAG_400)
...
void EnFdFire_Update(Actor* thisx, PlayState* play) {
    ...
    Actor_UpdateBgCheckInfo(play, &this->actor, 12.0f, 10.0f, 0.0f, UPDBGCHECKINFO_FLAG_1 | UPDBGCHECKINFO_FLAG_4);
...
void EnFdFire_Draw(Actor* thisx, PlayState* play) {
    ...
    if (1) {}
    if (1) {}
    Matrix_RotateYF(BINANG_TO_RAD((s16)(Camera_GetCamDirYaw(GET_ACTIVE_CAM(play)) + 0x8000)), MTXMODE_APPLY);
    Matrix_RotateZF(DEG_TO_RAD((sp88 * -10.0f) * sp80), MTXMODE_APPLY);
