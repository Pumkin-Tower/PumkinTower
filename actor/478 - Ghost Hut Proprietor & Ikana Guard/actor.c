...
asm(" \
    object_ps_Anim_00049C = 0x0600049C; \
    object_ps_Skel_007230 = 0x06007230; \
");
...
void func_80B0F6DC(EnGb2* this) {
    if (!CHECK_WEEKEVENTREG(WEEKEVENTREG_54_20)) {
        ...
        this->unk_26E = 0x14D0; // use 0x14DA for quick test
    } else {
        this->unk_26E = 0x14D1;
    }
    this->unk_288 = 30;
...
static void RouteTo(PlayState* play, int entrance)
{
    // workaround: reroute through exit-data
...
void EnGb2_Init(Actor* thisx, PlayState* play) {
    ...
    switch (ENGB2_GET_7(&this->actor)) {
        case ENGB2_7_0:
            ...
            if (gSaveContext.save.entrance == 1/*ENTRANCE(GHOST_HUT, 1)*/) {
