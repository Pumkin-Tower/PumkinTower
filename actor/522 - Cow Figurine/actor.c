...
#include "z64quake.h"
#include "objects/object_hariko/object_hariko.h"

asm(" \
	gHarikoBodyDL = 0x06002FE0; \
	gHarikoFaceDL = 0x06001E10; \
");
...
void ObjHariko_Init(Actor* thisx, PlayState* play) {
    ...
    //thisx->world.pos.y -= 8.5; // testing positioning in common clocktown maps
}
...
void ObjHariko_Draw(Actor* thisx, PlayState* play) {
    ...
    // draw body with no transformations
    gSPMatrix(POLY_OPA_DISP++, Matrix_NewMtx(play->state.gfxCtx), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
    gSPDisplayList(POLY_OPA_DISP++, gHarikoBodyDL);

    // bobblehead transformations
    Matrix_Push();
    Matrix_Translate(0, 85, 0, MTXMODE_APPLY); // move pumpkin head up into position
