...
void PolsVoice_Init(Actor* thisx, PlayState* play) {
    ...
    SkelAnime_InitFlex(play, &this->skelAnime, 0x06002918, 0x06002bcc, this->jointTable,
...
void PolsVoice_SetupIdle(PolsVoice* this, PlayState* play) {
    ...
    Animation_MorphToLoop(&this->skelAnime, 0x06002bcc, -6.0f);
    this->actionFunc = PolsVoice_Idle;
}

void PolsVoice_SetupHop(PolsVoice* this, PlayState* play) {
    Animation_MorphToLoop(&this->skelAnime, 0x06002e4c, 4.0f);
...
void PolsVoice_SetupStartGrab(PolsVoice* this, PlayState* play) {
    this->actor.speedXZ = 0.0f;
    this->actor.velocity.y = 0.0f;
    Animation_MorphToPlayOnce(&this->skelAnime, 0x060033e8, -3.0f);
...
void PolsVoice_SetupGnaw(PolsVoice* this, PlayState* play) {
    ...
    Animation_Change(&this->skelAnime, 0x060033e8, 1.0f, 0.0f, 12.0f, ANIMMODE_ONCE, 0.0f);
...
void PolsVoice_SetupEndGnaw(PolsVoice* this, PlayState* play) {
    ...
    this->actor.flags |= ACTOR_FLAG_0;
    Animation_MorphToPlayOnce(&this->skelAnime, 0x06002e4c, -4.0f);
    Audio_PlayActorSfx2(&this->actor, NA_SE_EN_RIZA_JUMP);
...
void PolsVoice_SetupDamaged(PolsVoice* this, PlayState* play) {
    ...
    this->actor.world.rot.y = this->actor.shape.rot.y = this->actor.yawTowardsPlayer;
    Animation_MorphToPlayOnce(&this->skelAnime, 0x06003650, -3.0f);
    this->actionFunc = PolsVoice_Damaged;
}

void PolsVoice_SetupStunned(PolsVoice* this, PlayState* play) {
    this->actor.speedXZ = 0.0f;
    Animation_Change(&this->skelAnime, 0x06003650, 0.0f, 2.0f, 0.0f, ANIMMODE_ONCE, 0.0f);
...
void PolsVoice_SetupDie(PolsVoice* this, PlayState* play) {
    ...
    this->actor.flags &= ~ACTOR_FLAG_0;
    this->actor.world.rot.y = this->actor.shape.rot.y = this->actor.yawTowardsPlayer;
    Animation_MorphToPlayOnce(&this->skelAnime, 0x060038d0, -3.0f);
