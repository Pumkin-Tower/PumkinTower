...
const ActorInit sEnBoB_InitVars = {
	...
	.flags        = ACTOR_FLAG_0 | ACTOR_FLAG_2 | ACTOR_FLAG_4 | ACTOR_FLAG_5,
	.objectId     = 0x0004,
	.instanceSize = sizeof(EnBoB),
	.init         = (ActorFunc)EnBoB_Init,
	.destroy      = (ActorFunc)EnBoB_Destroy,
	.update       = (ActorFunc)EnBoB_Update,
	.draw         = (ActorFunc)EnBoB_Draw
};
...
void EnBoB_UpdateDamage(EnBoB* this, PlayState* play) {
	...
	if (this->collider.base.acFlags & AC_HIT && this->colorTimer == 0 && this->boneHP > 0) {
		...
		if (this->boneHP > 0) {
			...
			Audio_PlayActorSfx2(&this->actor, NA_SE_EN_GOMA_EGG2);
	...
	if (this->bodycollider.base.acFlags & AC_HIT){
		...
		if (this->actor.colChkInfo.damageEffect == BoB_DMGEFF_NONE || this->actor.colChkInfo.damageEffect == BoB_DMGEFF_BONUS){
			...
			if ((this->hitcount == 3 || this->hitcount == 8 || this->hitcount == 12) && this->actor.health != 0 && this->numprojectile > 0){
...
void EnBoB_SetupDeath(EnBoB* this, PlayState* play) {
	if (this->actor.health == 0 && this->EnBoB_Dead == false){
...
void EnBoB_DislodgeBonesUpdateDraw(EnBoB* this) {
...
void EnBoB_ProjectileCollision(EnBoB* this, PlayState* play) {
	...
	for (int i = 0; i < BoBProjectileMax; i++) {
		...
		if (atkcol->base.at == &player->actor && p->isLanded != true){
			...
			if (info != NULL && info->toucher.dmgFlags == DMG_SHIELD && p->isDeflected == false){
				p->isDeflected = true;
	...
	for (int i = 0; i < BoBProjectileMax; ++i) {
		...
		if (p->isLanded == false){
			...
			if (!BgCheck_AnyLineTest1(&play->colCtx,&p->pos,&next,&result,&poly,true)){
				...
				p->isLanded = true;
				Audio_PlaySfxGeneral(NA_SE_EN_RIZA_DOWN,&p->pos, 4, &gSfxDefaultFreqAndVolScale,&gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
			}
		}
...
void EnBoB_Wander(EnBoB* this){
	...
	Math_SmoothStepToF(&this->actor.speedXZ, targetSpeed, 0.7f, 0.5f, 0.0f);
	Actor_MoveForward(&this->actor);
...
void EnBoB_Roll(EnBoB* this, PlayState* play){
	...
	Math_SmoothStepToF(&this->actor.speedXZ, targetSpeed, 0.7f, 0.5f, 0.0f);
	Actor_MoveForward(&this->actor);
	Math_SmoothStepToS(&this->actor.world.rot.y, Math_Vec3f_Yaw(&this->actor.pos, &this->rollTarget), 5, 0x1000, 0x80);
	EnBoB_PickupBones(this,play);
}

void EnBoB_PickupBones(EnBoB* this, PlayState* play){
	f32 distance = Math_Vec3f_DistXZ(&this->actor.pos, &this->rollTarget);
...
void EnBoB_UpdateCurrentBone(EnBoB* this) {
	...
	for (int i = 0; i < CurrentBoneMax; ++i) {
		...
		Matrix_RotateY_s(Math_Vec3f_Yaw(&gZeroVec, &pos), MTXMODE_APPLY);
		Matrix_RotateX_s(Math_Vec3f_Pitch(&gZeroVec, &pos), MTXMODE_APPLY);
		Matrix_RotateZ_f(Remap(curbone->roll, INTN_MIN(8), INTN_MAX(8), -180, 180), MTXMODE_APPLY);
		Matrix_Scale(0.01f, 0.01f, 0.01f, MTXMODE_APPLY);
		gSPMatrix(POLY_OPA_DISP++, NEW_MATRIX(), G_MTX_LOAD);
...
void EnBoB_Update(EnBoB* this, PlayState* play) {
	...
	Actor_UpdateBgCheckInfo(play, &this->actor, 30.0f, this->collider.dim.radius, 50.0f, UPDBGCHECKINFO_FLAG_0 | UPDBGCHECKINFO_FLAG_2);
	...
	if (this->EnBoB_Dead == false) {
		...
		if (this->func != EnBoB_Roll && this->func != EnBoB_SetupWander && this->timeStop == false){
			Math_ApproachS(&this->actor.shape.rot.y, Math_Vec3f_Yaw(&this->actor.pos, &player->actor.world.pos), 2, 1200);
			Math_ApproachS(&this->actor.world.rot.y, Math_Vec3f_Yaw(&this->actor.pos, &player->actor.world.pos), 2, 1200);
		}


		//turn actor to face roll target
		if (this->func == EnBoB_Roll){
			Math_ApproachS(&this->actor.shape.rot.y, Math_Vec3f_Yaw(&this->actor.pos, &this->rollTarget), 2, 1200);
			Math_ApproachS(&this->actor.world.rot.y, Math_Vec3f_Yaw(&this->actor.pos, &this->rollTarget), 2, 1200);
...
s32 EnBoB_OverrideLimbDraw(PlayState* play, s32 limbIndex, Gfx** dl, Vec3f* pos, Vec3s* rot, void* thisx, Gfx** gfx) {
	
	return 0;
}

void EnBoB_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dl, Vec3s* rot, void* thisx, Gfx** gfx) {
...
void EnBoB_Draw(EnBoB* this, PlayState* play) {
	...
	EnBoB_UpdateCurrentBone(this);
	EnBoB_DislodgeBonesUpdateDraw(this);
	...
	if (this->boneHP > 0) {
		...
		Matrix_Push();
		{
			if (this->timeStop != true)
				Matrix_RotateZ_s(play->gameplayFrames * 700, MTXMODE_APPLY);
			Matrix_Scale(0.01f, 0.01f, 0.01f, MTXMODE_APPLY);
		...
		if (this->timeStop != true)
			Matrix_RotateZ_s(play->gameplayFrames * -1200, MTXMODE_APPLY);
		Matrix_Scale(0.01f, 0.01f, 0.01f, MTXMODE_APPLY);
		gSPMatrix(POLY_XLU_DISP++, NEW_MATRIX(), G_MTX_LOAD);
